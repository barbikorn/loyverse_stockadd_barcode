"""
step2_stock_update.py — Step 2: Update stock in Loyverse

run(url, progress) emits progress events through the optional `progress` callback
so a UI can show live status (see step1_barcode_gen for the event shapes) and
...
def run(url: str | None = None, progress=None) -> dict:
    ...
    print("🚀 Starting Step 2: Stock Update")

    # Step 2 only touches rows that finished step 1 (state BARCODE_READY).
    todo = [
        (ws, row_idx, row)
        for ws, row_idx, row in input_data
        if row.get("state", "").upper() == "BARCODE_READY"
    ]
    total = len(todo)
    emit({"type": "start", "step": 2, "total": total, "file_name": file_name})
    ...
    for index, (ws, row_idx, row) in enumerate(todo, start=1):
        ...
        print(f"📦 [{cat}] ({index}/{total}) Updating Stock: {label} (SKU: {row['sku']})...")

        result = shared_logic.process_row_step2(row)
        report.append(shared_logic.to_transaction_record(row, result))
        status = result["Status"]

        if status == "Success":
            sheets.update_row_state(ws, row_idx, cols["state"], "COMPLETED")
        ...
        print(f"{icon} [{cat}] {label} | SKU: {row['sku']} | {status}")
        emit({
            "type": "item", "index": index, "total": total,
            "name": label, "category": cat, "sku": row["sku"],
    ...
    print(f"\n✨ Step 2 Completed: Processed {total} items.")
    summary = {"step": 2, "total": total, **counts}
